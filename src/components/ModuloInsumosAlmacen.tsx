...
export default function ModuloInsumosAlmacen() {
  ...
  const fetchInsumos = async () => {
    ...
    try {
      ...
      toast.error('Error al cargar catálogo de insumos.');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createMovimientoInsumo({
        ...
        sitio_instalacion: typeof formData.sitio_instalacion === 'string' && formData.sitio_instalacion.trim() !== '' ? formData.sitio_instalacion : null,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {isSalida && (
            <div className="flex flex-col gap-1.5 md:col-span-2 bg-orange-50 p-3 rounded-lg border border-orange-100">
              <label className="text-sm font-medium text-orange-800 flex items-center gap-1">
                <AlertCircle className="w-4 h-4" /> Sitio de Instalación / Justificación (Requerido)
              </label>
              <input
                type="text"
                required
                ...
                placeholder="Ej. Periférico Sur #200 o Reparación de taller"
                list="sitios_list"
              />
              <p className="text-xs text-orange-600 mt-1">Escribe la ubicación exacta o actividad en la que se utilizará para comprobar la salida.</p>
            </div>
          )}
      ...
      <div className="pt-4 flex justify-end">
        <button
          type="submit"
          disabled={loading || !formData.insumo_id}
        ...
        >
          ...
          {loading ? 'Guardando...' : (isSalida ? 'Registrar Salida' : 'Registrar Ingreso')}
        </button>
      </div>
    </form>
  );
